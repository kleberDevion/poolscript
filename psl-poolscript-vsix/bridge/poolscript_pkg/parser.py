...
class IndexAssignment(Node):
    """`alvo[indice] = valor`, e as formas aumentadas (`+=`, `-=`, ...).

    Separado do `Assignment` porque o alvo não é um nome: é um par
    (container, índice) que precisa ser avaliado antes de escrever. Aceita
    qualquer expressão como container, então `l[0][1] = x` e
    `obj.campo["k"] = v` caem aqui sem tratamento especial.
    """
    target: Node       # o container
    index: Node
    operator: str      # "=", "+=", ...
    value: Node
...
ASSIGN_OPS = {"=", "+=", "-=", "*=", "/=", "%="}
COMPARE_OPS = {"==", "!=", "===", "!==", "<", ">", "<=", ">="}
# Reservadas que o lexer NÃO marca como KW (são contextuais), mas que também
# não podem virar nome: ligar `base` a uma variável/parâmetro quebra a chamada
# de superclasse `base(...)` silenciosamente.
CONTEXTUAL_RESERVED = {"base"}
# `self` é KW mas é justamente o nome do primeiro parâmetro de um método —
# liberado só onde faz sentido (lista de parâmetros).
PARAM_ALLOWED_KEYWORDS = {"self"}
TYPE_KEYWORDS = {"str", "int", "flo", "bool", "list", "json", "dict", "tup", "type"}
EXPR_NAME_KEYWORDS = {"post", "input", "create", "clear", "space", "addEnd", "char", "list", "json", "dict", "tup", "JSON", "full", "mei", "self",
...
COUNT_TYPE_KEYWORDS = {"str", "int", "flo", "bool", "list", "json", "dict", "tup", "char"}
...
class Parser:
    def __init__(self, tokens: list[Token], source: str = "", filename: str = "<stdin>"):
        ...
        # Sem esta marca, `for each c in "abc" {` lia `"abc" {` como
        # interpolação e o bloco do laço sumia, obrigando a passar a string
        # por uma variável antes. A marca vale só no nível de cima da
        # expressão: dentro de (), [] ou {} a interpolação volta a valer.
        self.chave_abre_bloco = False
        # Profundidade de (), [] e {} atravessados. A marca acima só vale no
        # nível 0: dentro de um agrupamento não há bloco pra abrir, então
        # `for each x in ["a" {b}] { ... }` interpola o `{b}` e mesmo assim
        # deixa o `{` final abrir o laço.
        self.grupo_depth = 0
    ...
    def parse_statement(self) -> Node:
        ...
        # (`if = 5`, `action = 5`). Pega aqui, antes do dispatch por keyword,
        # senão o erro sai como "expressão inválida" — longe da causa real.
        # Só '=' puro: `x += 1` num nome reservado cai no mesmo caso, mas
        # comparação (`==`) é expressão legítima e não entra aqui.
        if tok.type == "KW" and self.pos + 1 < len(self.tokens):
            nxt = self.tokens[self.pos + 1]
            if nxt.type == "OP" and nxt.value in ASSIGN_OPS:
                raise self.error(
                    f"'{tok.value}' é palavra reservada da linguagem e não pode ser "
                    f"usada como nome de variável",
                    tok,
                )
        ...
        # `alvo[i] = v`. Detectado DEPOIS de montar a expressão, não por
        # lookahead: o alvo pode ser qualquer coisa indexável (`l[0][1]`,
        # `obj.d["k"]`), e reconhecer isso à frente exigiria varrer colchetes
        # balanceados. Aqui o parser de expressão já fez esse trabalho.
        if (isinstance(expr, IndexAccess)
                and self.current().type == "OP"
                and self.current().value in ASSIGN_OPS):
            op_tok = self.current()
            self.pos += 1
            ...
            val = self.parse_expression()
            self.consume_optional_semi()
            return IndexAssignment(line=expr.line, col=expr.col, target=expr.target,
                                   index=expr.index, operator=str(op_tok.value), value=val)
    ...
    def parse_var_decl(self) -> VarDecl:
        ...
        name_tok = self.expect_name("variável", msg="nome de variável inválido")
    ...
    def parse_assignment(self) -> Assignment:
        name_tok = self.expect_name("variável")
    ...
    def parse_unpack_target_list(self) -> UnpackTarget:
        ...
        def parse_one() -> None:
            ...
            if tok.type == "OP" and tok.value == "*":
                ...
                self.pos += 1
                name_tok = self.expect_name("variável", msg="esperado nome após '*' em desempacotamento")
            ...
            if tok.type == "LPAREN":
                ...
                return
            name_tok = self.expect_name("variável", msg="alvo de desempacotamento inválido")
            elements.append(str(name_tok.value))
    ...
    def parse_for_stmt(self) -> ForEachStmt:
        ...
        name_tok = self.expect_name("variável de loop", msg="nome de variável inválido")
        self.expect("KW", "in", "esperado 'in' no loop for each")
        salvo = self.chave_abre_bloco
        self.chave_abre_bloco = True
        try:
            iterable = self.parse_expression()
        finally:
            self.chave_abre_bloco = salvo
        block = self.parse_block()
    ...
    def parse_action_decl(self) -> ActionDecl:
        ...
        name_tok = self.expect_name("action", msg="nome de action inválido")
        self.expect("LPAREN")
        params: list[str] = []
        defaults: dict = {}
        if self.current().type != "RPAREN":
            while True:
                ...
                if self.current().type == "KW" and self.current().value == "self":
                    ...
                    param_name = self.parse_bindable_name("parâmetro", msg="parâmetro inválido")
    ...
    def parse_model_decl(self) -> "ModelDecl":
        ...
        name_tok = self.expect_name("model", msg="esperado nome do model após 'model'")
        ...
        while self.current().type != "RBRACE":
            # campo: tipo(length=N)
            field_tok = self.expect_name("campo", msg="esperado nome do campo")
    ...
    def parse_entity_decl(self) -> "EntityDecl":
        ...
        self.pos += 1
        name_tok = self.expect_name("Entity", msg="esperado nome da Entity após 'Entity'")
        name = str(name_tok.value)
    ...
    def _parse_entity_field(self):
        ...
        if tok.type not in {"IDENT", "IDENT_UPPER"}:
            return None
        # lookahead: próximo token deve ser COLON — se não for, isto não é um
        # campo válido. Levanta erro em vez de devolver None: um None aqui
        # chega ao chamador (parse_entity_decl) sem ter consumido nenhum
        # token, e como o laço de lá só reage a `if ef:`, o loop `while`
        # ficava rodando pra sempre no mesmo token (trava o parser/bridge
        # em CPU 100%, sem nunca terminar nem dar erro).
    ...
    def parse_global_stmt(self) -> GlobalStmt:
        ...
        names = [self.parse_bindable_name("variável", "esperado nome de variável após 'global'")]
        while self.match("COMMA"):
            names.append(self.parse_bindable_name("variável", "esperado nome de variável após ','"))
        self.consume_optional_semi()
        return GlobalStmt(line=start.line, col=start.col, names=names)

    def expect_name(self, kind: str, msg: str | None = None) -> Token:
        """`expect` de um nome que vai ser LIGADO (variável, parâmetro, action,
        Entity, ...), recusando palavra reservada com mensagem clara — igual o
        Python recusa `if = 1`.

        Sem isto, reservada em posição de nome dava erro solto e distante
        ('esperado ( após base', 'expressão inválida') ou, pior, passava
        silenciosamente: `base = 5` era aceito e só quebrava depois, na
        chamada de superclasse, como se a variável não existisse.
        """
        tok = self.current()
        if tok.type == "KW" or (tok.type in {"IDENT", "IDENT_UPPER"}
                                and tok.value in CONTEXTUAL_RESERVED):
            raise self.error(
                f"'{tok.value}' é palavra reservada da linguagem e não pode ser "
                f"usada como nome de {kind}",
                tok,
            )
        return self.expect(("IDENT", "IDENT_UPPER"), msg=msg or f"esperado nome de {kind}")

    def parse_bindable_name(self, kind: str, msg: str | None = None) -> str:
        """Igual a `expect_name`, mas devolve só o texto do nome."""
        return str(self.expect_name(kind, msg).value)
    ...
    def parse_compare(self) -> Node:
        ...
        while True:
            ...
            if tok.type == "OP" and tok.value in COMPARE_OPS:
                self.pos += 1
                node = BinaryOp(line=tok.line, col=tok.col, left=node, operator=str(tok.value), right=self.parse_bitor())
                continue
            if tok.type == "KW" and tok.value in {"is", "in"}:
                self.pos += 1
                if tok.value == "is" and self.current().type == "KW" and self.current().value in {"not", "Not"}:
                    self.pos += 1
                    node = BinaryOp(line=tok.line, col=tok.col, left=node, operator="is not", right=self.parse_bitor())
                else:
                    node = BinaryOp(line=tok.line, col=tok.col, left=node, operator=str(tok.value), right=self.parse_bitor())
            ...
            if tok.type == "KW" and tok.value in {"not", "Not"} and self.peek().type == "KW" and self.peek().value in {"is", "in"}:
                ...
                node = BinaryOp(line=tok.line, col=tok.col, left=node, operator=op, right=self.parse_bitor())
                continue
            break
        return node

    # Cadeia bitwise — mesma ordem de precedência do Python (mais fraco
    # pro mais forte): | , ^ , & , << / >>. Todos mais fortes que comparação
    # e mais fracos que soma/subtração, igual ao Python.
    def parse_bitor(self) -> Node:
        node = self.parse_xor()
        while self.current().type == "OP" and self.current().value == "|":
            tok = self.current()
            self.pos += 1
            node = BinaryOp(line=tok.line, col=tok.col, left=node, operator="|", right=self.parse_xor())
        return node
    ...
    def parse_xor(self) -> Node:
        ...
        while self.current().type == "OP" and self.current().value == "^":
            tok = self.current()
            self.pos += 1
            node = BinaryOp(line=tok.line, col=tok.col, left=node, operator="^", right=self.parse_bitand())
        return node
    ...
    def parse_bitand(self) -> Node:
        ...
        while self.current().type == "OP" and self.current().value == "&":
            tok = self.current()
            self.pos += 1
            node = BinaryOp(line=tok.line, col=tok.col, left=node, operator="&", right=self.parse_shift())
        return node
    ...
    def parse_call_args_until(self, closing_type: str) -> list[CallArg]:
        ...
        self.grupo_depth += 1
        try:
            return self._call_args_corpo(closing_type, args)
        finally:
            self.grupo_depth -= 1

    def _call_args_corpo(self, closing_type: str, args: list[CallArg]) -> list[CallArg]:
        self.skip_separators()
        while self.current().type != closing_type:
            # Nome de argumento nomeado é só um RÓTULO — pode ser palavra
            # reservada. A stdlib depende disso: `regex.sub(..., count=2)` e
            # `.replace(..., count=2)` expõem um kwarg chamado `count`, que é
            # keyword da linguagem; sem aceitar KW aqui esses parâmetros eram
            # impossíveis de passar (só posicional). O `=` logo depois
            # desambigua de qualquer uso do keyword como operador — `count`
            # infixo (`int(7) count in xs`) nunca é seguido de '='.
            if (self.current().type in {"IDENT", "IDENT_UPPER", "KW"}
                    and self.peek().type == "OP" and self.peek().value == "="):
    ...
    def parse_primary(self) -> Node:
        ...
        if tok.type == "KW" and tok.value in {"action", "reaction"} and self.peek().type == "LPAREN":
            ...
            while self.current().type != "RPAREN":
                p = self.expect_name("parâmetro", msg="esperado nome de parâmetro")
    ...
    def parse_string_like(self) -> Node:
        ...
        if self.current().type != "LBRACE" or (self.chave_abre_bloco and self.grupo_depth == 0):
            return parts[0]
        ...
        if self.current().type != "RBRACE":
            while True:
                ...
                if key_tok.type == "STR":
                    ...
                    # Qualquer expressão serve de chave — `{1: "a"}` tem que
                    # valer, já que `d[1] = "a"` sempre valeu. Aceitar só
                    # string/identificador deixava as duas formas de escrever
                    # a mesma coisa em desacordo.
