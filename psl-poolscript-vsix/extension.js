// PoolScript — extensão VS Code (type-aware, estilo Pylance).
//
// Três fontes de símbolos, do mais específico pro mais geral:
//   1. metadata.json  — a stdlib inteira, introspectada de verdade
//      (bridge/gen_metadata.py): cada função com PARÂMETROS e cada classe com
//      MEMBROS e o TIPO DE RETORNO. É o que dá a cadeia real:
//         conn = psodbc.connect(...)  -> DbConnection
//         cur  = conn.cursor()        -> DbCursor  -> fetchall/fetchone/...
//   2. libs instaladas — .ps/.psl em ~/.poolscript/libs (ou $POOLSCRIPT_HOME/libs)
//   3. o arquivo atual + imports locais — actions, Entities/classes, enums, vars
//
// Regra de ouro (o que motivou reescrever do zero): se o tipo de uma variável
// NÃO é conhecido, o editor NÃO inventa membro nenhum. Nada de `.cursor` verde
// numa var que não é conexão. Ou sabe o tipo, ou não sugere.
...
function parsePoolSource(text) {
  const sym = { functions: [], classes: {}, enums: [], vars: [] };
...
function completionDeTopo(doc) {
  ...
  for (const k of KEYWORDS)
    itens.push(new vscode.CompletionItem(k, vscode.CompletionItemKind.Keyword));
  return itens;
}
...
const hoverProvider = {
  provideHover(doc, pos) {
    ...
    const linhaAteCursor = doc.lineAt(pos.line).text.slice(0, range.start.character);

    // membro depois de '.': resolve o tipo do receptor e mostra a assinatura
    const recv = receptorAntesDoPonto(linhaAteCursor + (linhaAteCursor.endsWith('.') ? '' : '.'));
    if (linhaAteCursor.trimEnd().endsWith('.') || recv) {
      const base = receptorAntesDoPonto((linhaAteCursor.trimEnd().endsWith('.')) ? linhaAteCursor : linhaAteCursor + '.');
      if (base) {
        const tipo = resolveChain(parseChain(base), doc, pos.line);
        const membros = membrosDoTipo(tipo, doc);
        const m = membros && membros.find(x => x.name === palavra);
        if (m) return hoverDeMembro(m, tipo);
      }
    }
    // módulo?
    if (META.modules[palavra]) {
      return new vscode.Hover(new vscode.MarkdownString(`**${palavra}** — módulo da stdlib`));
    }
    // keyword?
    if (KEYWORDS.includes(palavra)) {
      return new vscode.Hover(new vscode.MarkdownString(`\`${palavra}\` — palavra-chave da PoolScript`));
    }
    return null;
  },
};

function hoverDeMembro(m, tipo) {
  const md = new vscode.MarkdownString();
  const dono = tipo.module || tipo.cls || tipo.scalar || '';
  const sig = m.sig || (m.params ? `${m.name}(${m.params.map(p => p.name + (p.opt ? '?' : '')).join(', ')})` : m.name);
  md.appendCodeblock(`${dono ? dono + '.' : ''}${sig}${m.returns ? ' -> ' + m.returns : ''}`, 'poolscript');
  md.appendMarkdown(`\n${m.kind === 'property' ? 'propriedade' : m.kind}`);
