...
// ─────────────────────────────────────────────────────────────────────────
// Marca import/variável NÃO USADOS (apagados, como o Pylance). Regra: um nome
// declarado que aparece só UMA vez no arquivo (a própria declaração) não é usado.
function diagnosticosNaoUsados(doc) {
  const texto = doc.getText();
  const linhas = texto.split('\n');
  const diags = [];
  const conta = (nome) => (texto.match(new RegExp('\\b' + nome.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\b', 'g')) || []).length;
  const marca = (nome, linha, col, oque) => {
    if (conta(nome) > 1) return;                 // usado em algum outro ponto
    const r = new vscode.Range(linha, col, linha, col + nome.length);
    const d = new vscode.Diagnostic(r, `${oque} '${nome}' nunca é usado`, vscode.DiagnosticSeverity.Hint);
  ...
  for (let i = 0; i < linhas.length; i++) {
    const ln = linhas[i];
    const semC = ln.replace(/(#|\/\/).*$/, '');
    // import X as Y  -> Y
    let m = semC.match(/^\s*import\s+[\w.]+\s+as\s+([A-Za-z_]\w*)/);
    if (m) { marca(m[1], i, ln.lastIndexOf(m[1]), 'import'); continue; }
    // from PATH import a, b as c  -> cada nome local importado
    const fr = semC.match(/^\s*from\s+[.\w/]+\s+import\s+(.+)$/);
    if (fr) {
      const re = /([A-Za-z_]\w*)(?:\s+as\s+([A-Za-z_]\w*))?/g; let mm;
      while ((mm = re.exec(fr[1]))) {
        if (mm[1] === 'as') continue;
        const local = mm[2] || mm[1];
        marca(local, i, ln.indexOf(local, ln.indexOf('import') + 6), 'import');
      }
      continue;
    }
...
function ativaEmbutido(ctx) {
  ...
  // diagnostics de não-usado — guardado: se a API não existir (harness de teste),
  // simplesmente não liga, sem quebrar a ativação.
