...
- uma **`funct`** que roda quando alguém acessa esse caminho e devolve a
  resposta.
...
navegador pede  GET /api/hello
        │
        ▼
jinker acha a rota "/api/hello"  ──► roda a sua funct  ──► você faz `return ...`
        │
        ▼
jinker transforma seu return em resposta HTTP  ──►  navegador recebe
```

Dentro da funct você tem dois objetos de graça (não precisa declarar):
...
2. **`@app.route("/", ...)`** registra a `funct` logo abaixo como a resposta
   pra `GET /`. Você **não** chama `inicio()` — o jinker chama por você quando
   a URL é acessada.
3. Dentro da funct, **`request`** já existe. `return jsonify(...)` vira a
...
Não precisa envolver a `funct` em chaves extras — o `@app.route(...)` já captura a próxima `funct` como handler da rota. A `funct` não recebe parâmetros — `request` já está disponível automaticamente dentro dela.
...
O casamento é por nome, não por posição: numa rota `/x:a/sub:b`, uma funct
...
a `funct` continua síncrona, sem `async`/`await`. A diferença aparece quando um
