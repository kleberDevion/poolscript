# `menu(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `menu` — Lista de comandos/ações — contêiner de `li` ou `button`s.

É **contêiner**: `x = app.menu()` e depois `x.p()`, `x.entry()`, `x.button()`...
...
- `type()` devolve `"menu"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `menu` |
...
app = guzer.UI("Demo")
m = app.menu()
m.button(onclick=scripts.Novo).text("Novo")
m.button(onclick=scripts.Abrir).text("Abrir")
```
