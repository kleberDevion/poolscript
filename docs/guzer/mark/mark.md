# `mark(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `mark` — Texto destacado (como marca-texto) — o termo encontrado numa busca, por exemplo.

> No HTML `mark` é **inline** (fica na mesma linha do texto ao redor). No guzer
...
- `type()` devolve `"mark"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `mark` |
...
app = guzer.UI("Demo")
app.mark().stylesheet({ "background": "#fff176" }).text("resultado")
```
