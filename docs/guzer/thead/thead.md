...
Elemento `thead` — Grupo das linhas de **cabeçalho** da tabela (as `tr` com `th`).

É **contêiner**: `x = app.thead()` e depois `x.p()`, `x.entry()`, `x.button()`...
...
- `type()` devolve `"thead"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `thead` |
