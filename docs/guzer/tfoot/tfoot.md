...
Elemento `tfoot` — Grupo das linhas de **rodapé** da tabela (totais).

É **contêiner**: `x = app.tfoot()` e depois `x.p()`, `x.entry()`, `x.button()`...
...
- `type()` devolve `"tfoot"` na janela nativa (X11).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `tfoot` |
