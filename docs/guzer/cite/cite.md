# `cite(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `cite` — Título de uma obra citada — livro, filme, artigo, autor da citação.

> No HTML `cite` é **inline** (fica na mesma linha do texto ao redor). No guzer
...
- `type()` devolve `"cite"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `cite` |
