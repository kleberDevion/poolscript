...
Elemento `a` — link. Caixa com texto; o `href=` é aceito e guardado (a navegação ao clicar ainda não abre o navegador — use `onclick=` com uma reaction sua se quiser reagir ao clique).

## Como todo elemento do guzer

- Vem do `app` (o `guzer.UI`): `app.a(...)`.
- `type()` devolve `"a"` — igual nos dois motores.
...
import guzer
app = guzer.UI("Demo")
app.a().stylesheet({ "width": "320" }).text("conteúdo")
```
