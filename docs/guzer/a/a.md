# `a(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `a` — Link. `href=` é o destino e `target=` a janela-alvo. **Hoje o guzer não abre o navegador ao clicar**: o `href=` fica guardado no elemento (você pode lê-lo no handler) e o clique dispara o `onclick=`, se você passar um.

> No HTML `a` é **inline** (fica na mesma linha do texto ao redor). No guzer
...
- `type()` devolve `"a"` nos dois motores (tkinter no interpretador, X11 no `pool`).
- `.stylesheet({...})` e `.text(...)` encadeiam (devolvem o próprio elemento).

## Atributos

No HTML, os atributos próprios de `a` são `href=`, `target=`.
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `a` |
...
app = guzer.UI("Demo")
app.a(href="https://poolscript.dev", onclick=scripts.AbrirSite).text("site oficial")
```
