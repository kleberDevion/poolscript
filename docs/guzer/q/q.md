# `q(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `q` — Citação curta, inline — em HTML ganha aspas automáticas; aqui escreva-as no texto se quiser.

> No HTML `q` é **inline** (fica na mesma linha do texto ao redor). No guzer
...
- `type()` devolve `"q"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `q` |
