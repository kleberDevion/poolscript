...
Elemento `q` — uma **caixa empilhada com texto** na janela nativa. Os elementos estruturais do HTML têm todos o mesmo comportamento visual no guzer: caixa + texto; o que muda é a SEMÂNTICA que o seu código expressa.

## Como todo elemento do guzer

- Vem do `app` (o `guzer.UI`): `app.q(...)`.
- `type()` devolve `"q"` — igual nos dois motores.
...
import guzer
app = guzer.UI("Demo")
app.q().stylesheet({ "width": "320" }).text("conteúdo")
```
