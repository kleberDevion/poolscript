# `span(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `span` — Trecho inline genérico — marca um pedaço de texto sem significado próprio (o equivalente inline da `div`).

> No HTML `span` é **inline** (fica na mesma linha do texto ao redor). No guzer
...
- `type()` devolve `"span"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `span` |
...
app = guzer.UI("Demo")
app.span().stylesheet({ "color": "#c0392b" }).text("destaque")
```
