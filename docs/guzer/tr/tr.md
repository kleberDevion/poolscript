...
Elemento `tr` — Linha da tabela — contêiner de `td`/`th`.

É **contêiner**: `x = app.tr()` e depois `x.p()`, `x.entry()`, `x.button()`...
...
- `type()` devolve `"tr"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `tr` |
