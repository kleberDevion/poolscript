...
Elemento `sup` — Sobrescrito — expoentes (x²), notas de rodapé, ordinais.

> No HTML `sup` é **inline** (fica na mesma linha do texto ao redor). No guzer
...
- `type()` devolve `"sup"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `sup` |
