# `small(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `small` — Texto secundário/miúdo — letras miúdas, avisos legais, notas de rodapé.

> No HTML `small` é **inline** (fica na mesma linha do texto ao redor). No guzer
...
- `type()` devolve `"small"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `small` |
...
app = guzer.UI("Demo")
app.small().text("* sujeito a alteração sem aviso")
```
