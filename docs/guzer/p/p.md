# `p(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `p` — Parágrafo de texto — o elemento de texto corrido mais comum.
...
- `type()` devolve `"p"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `p` |
...
app = guzer.UI("Demo")
app.p().stylesheet({ "width": "400", "height": "60" }).text("Um parágrafo inteiro de texto na janela.")
```
