...
Elemento `caption` — Título/legenda da tabela — o primeiro filho do `table`.
...
- `type()` devolve `"caption"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `caption` |
