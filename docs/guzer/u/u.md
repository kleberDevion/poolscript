# `u(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `u` — Sublinhado — anotação não-textual (ex.: marcar um erro ortográfico). Não use pra links.

> No HTML `u` é **inline** (fica na mesma linha do texto ao redor). No guzer
...
- `type()` devolve `"u"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `u` |
