...
Elemento `h3` — Título de nível 3 — subseção.
...
- `type()` devolve `"h3"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `h3` |
...
app = guzer.UI("Demo")
app.h3().stylesheet({ "font-size": "17" }).text("Subseção")
```
