...
Elemento `sub` — Subscrito — o `2` de H₂O, índices.

> No HTML `sub` é **inline** (fica na mesma linha do texto ao redor). No guzer
...
- `type()` devolve `"sub"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `sub` |
