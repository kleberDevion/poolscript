# `blockquote(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `blockquote` — Citação em bloco — trecho vindo de outra fonte. Ponha a origem num `cite` filho.

É **contêiner**: `x = app.blockquote()` e depois `x.p()`, `x.entry()`, `x.button()`...
...
- `type()` devolve `"blockquote"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `blockquote` |
...
app = guzer.UI("Demo")
bq = app.blockquote()
bq.p().text("Simplicidade é o último grau de sofisticação.")
bq.cite().text("Leonardo da Vinci")
```
