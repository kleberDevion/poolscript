# `ins(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `ins` — Texto inserido numa revisão (em HTML, sublinhado). Par do `del`.

> No HTML `ins` é **inline** (fica na mesma linha do texto ao redor). No guzer
...
- `type()` devolve `"ins"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `ins` |
