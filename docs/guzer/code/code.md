# `code(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `code` — Trecho de código — em HTML, fonte monoespaçada. Aqui a caixa mostra o texto tal como veio.

> No HTML `code` é **inline** (fica na mesma linha do texto ao redor). No guzer
...
- `type()` devolve `"code"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `code` |
