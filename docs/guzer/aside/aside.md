# `aside(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `aside` — Conteúdo lateral/relacionado — barra lateral, nota à margem, links relacionados.

É **contêiner**: `x = app.aside()` e depois `x.p()`, `x.entry()`, `x.button()`...
...
- `type()` devolve `"aside"` na janela nativa (X11).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `aside` |
...
app = guzer.UI("Demo")
lado = app.aside()
lado.h4().text("Veja também")
lado.a(href="/docs").text("Documentação")
```
