...
Elemento `var` — Variável — de matemática ou de programa (`x`, `total`).

> No HTML `var` é **inline** (fica na mesma linha do texto ao redor). No guzer
...
- `type()` devolve `"var"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `var` |
