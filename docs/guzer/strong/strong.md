# `strong(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `strong` — Importância forte — em HTML vira negrito. No guzer a fonte é a do sistema (sem negrito hoje); o que a tag carrega é a **semântica**.

> No HTML `strong` é **inline** (fica na mesma linha do texto ao redor). No guzer
...
- `type()` devolve `"strong"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `strong` |
...
app = guzer.UI("Demo")
app.strong().text("Atenção: salve antes de sair")
```
