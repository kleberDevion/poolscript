# `i(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `i` — Itálico de estilo — termo técnico, pensamento, nome de navio, palavra em outra língua. Prefira `em` pra ênfase.

> No HTML `i` é **inline** (fica na mesma linha do texto ao redor). No guzer
...
- `type()` devolve `"i"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `i` |
