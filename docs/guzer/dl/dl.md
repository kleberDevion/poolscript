# `dl(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `dl` — Lista de definições — pares `dt` (termo) / `dd` (descrição), como um glossário.

É **contêiner**: `x = app.dl()` e depois `x.p()`, `x.entry()`, `x.button()`...
...
- `type()` devolve `"dl"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `dl` |
