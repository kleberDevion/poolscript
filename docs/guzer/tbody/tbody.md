...
Elemento `tbody` — Grupo das linhas do **corpo** da tabela.

É **contêiner**: `x = app.tbody()` e depois `x.p()`, `x.entry()`, `x.button()`...
...
- `type()` devolve `"tbody"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `tbody` |
