# `abbr(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `abbr` — Abreviação ou sigla (`PS`, `HTML`). Em HTML a expansão vai no `title`; aqui guarde-a em `value=` pra ler depois.

> No HTML `abbr` é **inline** (fica na mesma linha do texto ao redor). No guzer
...
- `type()` devolve `"abbr"` nos dois motores (tkinter no interpretador, X11 no `pool`).
- `.stylesheet({...})` e `.text(...)` encadeiam (devolvem o próprio elemento).

## Atributos

No HTML, os atributos próprios de `abbr` são `value=`.
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `abbr` |
...
app = guzer.UI("Demo")
app.abbr(value="PoolScript").text("PS")
```
