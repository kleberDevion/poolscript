...
Elemento `summary` — Título de um `details` — a linha que, no HTML, abre e fecha o bloco.
...
- `type()` devolve `"summary"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `summary` |
