# `hr(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `hr` — Quebra temática — a linha horizontal que separa blocos. Sem `.text()` é uma caixa vazia; dê `height` pequena e uma cor pra virar uma linha de verdade.
...
- `type()` devolve `"hr"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `hr` |
...
app = guzer.UI("Demo")
app.hr().stylesheet({ "height": "2", "width": "400", "background": "#cccccc" })
```
