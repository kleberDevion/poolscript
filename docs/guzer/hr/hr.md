...
Elemento `hr` — separador. É uma caixa como as demais; dê a ele a altura/cor que quiser no `.stylesheet` (ex.: `height: 2` faz uma linha).

## Como todo elemento do guzer

- Vem do `app` (o `guzer.UI`): `app.hr(...)`.
- `type()` devolve `"hr"` — igual nos dois motores.
...
import guzer
app = guzer.UI("Demo")
app.hr().stylesheet({ "width": "320" }).text("conteúdo")
```
