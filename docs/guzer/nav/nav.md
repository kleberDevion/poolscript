# `nav(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `nav` — Bloco de navegação — agrupa os links (`a`) principais do app.

É **contêiner**: `x = app.nav()` e depois `x.p()`, `x.entry()`, `x.button()`...
...
- `type()` devolve `"nav"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `nav` |
...
app = guzer.UI("Demo")
menu = app.nav()
menu.a(href="/").text("Início")
menu.a(href="/sobre").text("Sobre")
```
