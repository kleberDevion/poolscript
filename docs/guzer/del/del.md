# `del(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `del` — Texto removido numa revisão (em HTML, riscado). Combina com `ins` pra mostrar o antes/depois.

> No HTML `del` é **inline** (fica na mesma linha do texto ao redor). No guzer
...
- `type()` devolve `"del"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `del` |
...
app = guzer.UI("Demo")
app.del().text("prazo: 10 dias")
app.ins().text("prazo: 5 dias")
```
