# `ol(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `ol` — Lista **ordenada** — contêiner de `li`. A numeração não é automática aqui: escreva-a no `.text()`.

É **contêiner**: `x = app.ol()` e depois `x.p()`, `x.entry()`, `x.button()`...
...
- `type()` devolve `"ol"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `ol` |
