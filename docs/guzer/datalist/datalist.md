# `datalist(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `datalist` — Lista de sugestões pra um campo (`option`s filhos). **Hoje é só o modelo**: o `entry` não puxa as sugestões dela.

É **contêiner**: `x = app.datalist()` e depois `x.p()`, `x.entry()`, `x.button()`...
...
- `type()` devolve `"datalist"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `datalist` |
...
app = guzer.UI("Demo")
dl = app.datalist(name="cidades")
dl.option(value="Recife")
dl.option(value="Natal")
```
