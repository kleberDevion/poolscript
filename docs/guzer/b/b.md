# `b(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `b` — Negrito de estilo, **sem** importância semântica (nomes de produto, palavras-chave). Prefira `strong` quando o texto for importante de verdade.

> No HTML `b` é **inline** (fica na mesma linha do texto ao redor). No guzer
...
- `type()` devolve `"b"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `b` |
