# `header(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `header` — Cabeçalho de página ou de seção — título, logo, navegação inicial. Vai no topo da pilha.

É **contêiner**: `x = app.header()` e depois `x.p()`, `x.entry()`, `x.button()`...
...
- `type()` devolve `"header"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `header` |
...
app = guzer.UI("Demo")
topo = app.header()
topo.img(src="logo.png")
topo.h1().text("Meu app")
```
