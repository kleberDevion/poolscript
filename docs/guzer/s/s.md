# `s(typeinp=, placeholder=, value=, name=, href=, src=, alt=, target=, forid=, action=, methd=, rows=, cols=, onclick=)`

Elemento `s` — Texto riscado — conteúdo que não vale mais (preço antigo, item cancelado).

> No HTML `s` é **inline** (fica na mesma linha do texto ao redor). No guzer
...
- `type()` devolve `"s"` nos dois motores (tkinter no interpretador, X11 no `pool`).
...
| `href=`, `src=`, `alt=`, `target=`, `forid=`, `action=`, `methd=`, `rows=`, `cols=`, `typeinp=` | aceitos e guardados no elemento; **sem efeito visual** em `s` |
