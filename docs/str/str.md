...
| [`get`](get/get.md) | `s.get(chave=Null)` | Interpreta como JSON de objeto e devolve o valor da chave; sem chave, o objeto inteiro. |
| [`get_json`](get_json/get_json.md) | `s.get_json(chave=Null)` | Interpreta a string como JSON e devolve o valor. |
...
| [`ljust`](ljust/ljust.md) | `s.ljust(width, fillchar)` | Preenche à direita até a largura. |
...
| [`replace`](replace/replace.md) | `s.replace(old, new, count)` | Troca ocorrências; `old` pode ser LISTA de alvos. |
...
| [`rjust`](rjust/rjust.md) | `s.rjust(width, fillchar)` | Preenche à esquerda até a largura. |
| [`rpartition`](rpartition/rpartition.md) | `s.rpartition(sep)` | Tupla (antes, sep, depois) na ÚLTIMA ocorrência. |
| [`rsplit`](rsplit/rsplit.md) | `s.rsplit(sep=Null, maxsplit=-1)` | Como split, mas conta as divisões da DIREITA. |
| [`rstrip`](rstrip/rstrip.md) | `s.rstrip(chars=Null)` | Remove da ponta DIREITA. |
| [`split`](split/split.md) | `s.split(sep=Null, maxsplit=-1)` | Divide em lista; sem separador, divide por espaços. |
...
| [`sub`](sub/sub.md) | `s.sub(pattern, repl)` | Substitui as ocorrências do padrão regex. |
