...
def test_install_command_creates_shim_and_tracks(tmp_path, monkeypatch):
    ...
    # o shim tem nome diferente por plataforma: `.cmd` no Windows, script
    # `sh` sem extensão no resto — checar só o `.cmd` reprovava no Linux.
    shim = paths.bin / ("hello.cmd" if os.name == "nt" else "hello")
    assert shim.is_file()
