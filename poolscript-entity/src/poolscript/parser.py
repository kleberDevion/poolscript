...
class Parser:
    ...
    def expect(self, type_: str | tuple, value: str | None = None, msg: str | None = None) -> Token:
        tok = self.current()
        ...
        raise self.error(msg or f"esperado {expected}, encontrou {tok.type}:{tok.value!r}", tok)

    def error(self, msg: str, token: Token | None = None) -> PoolParseError:
        return PoolParseError(msg, token or self.current(), self.source)
    ...
    def parse_decorator_stmt(self, capture_action: bool = True) -> DecoratorStmt:
        ...
        if self.current().type in {"COLON", "LBRACE"}:
            block = self.parse_block()
        elif capture_action and self.current().type == "KW" and self.current().value == "action":
        ...
        if self.match("LPAREN"):
            ...
            if self.match("COMMA"):
                ...
                while self.current().type != "RPAREN":
                    ...
                    if not self.match("COMMA"):
                        break
                self.expect("RPAREN", msg="faltou ')' na tupla")
                return TupleLiteral(line=tok.line, col=tok.col, items=items)
            self.expect("RPAREN", msg="faltou ')' na expressão")
            return expr
        if self.match("LBRACK"):
